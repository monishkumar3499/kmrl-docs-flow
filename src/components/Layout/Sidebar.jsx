...
export const Sidebar = () => {
  ...
  // Only show departments for admin users
  const filteredDepartmentItems = user?.role === 'admin' ? departmentItems : [];

  const getDepartmentColor = (department) => {
    if (!department) return 'bg-primary';
    
    const dept = department.toLowerCase();
    if (dept.includes('operations')) return 'bg-blue-500';
    if (dept.includes('engineering')) return 'bg-purple-500';
    if (dept.includes('finance')) return 'bg-green-500';
    if (dept.includes('human') || dept.includes('hr')) return 'bg-orange-500';
    if (dept.includes('safety')) return 'bg-red-500';
    return 'bg-primary';
  };
    ...
    <div className="h-full bg-card border-r border-border flex flex-col shadow-lg">
      ...
      <div className="p-6 border-b border-border">
        <div className="flex items-center gap-3">
          <img src="/kmrl-logo.png" alt="KMRL" className="w-8 h-8" />
          <div>
      ...
      <nav className="flex-1 p-4 overflow-y-auto">
              ...
              return (
                <Link key={item.href} to={item.href}>
                  <div className={cn(
                    ...
                    isActive 
                      ? "bg-gradient-to-r from-primary to-orange-400 text-white shadow-lg" 
        ...
        {/* Departments Navigation - Only for Admin */}
        {filteredDepartmentItems.length > 0 && (
          <div className="mb-4">
            <h3 className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-3">
              DEPARTMENTS
            </h3>
